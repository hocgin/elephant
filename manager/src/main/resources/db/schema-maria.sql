...
(
  ...
  COMMENT = '[用户模块] 员工表';
ALTER TABLE `staff`
  ADD UNIQUE INDEX index_unique_username (`username`);
...
# 不能加唯一索引, 因为会通过更新来矫正唯一性, 插入时并不能保证唯一
ALTER TABLE `resource`
  ADD INDEX index_lft (`lft`);
ALTER TABLE `resource`
  ADD INDEX index_rgt (`rgt`);
...
(
  ...
  COMMENT = '[权限模块] 角色表';
ALTER TABLE `role`
  ADD UNIQUE INDEX index_unique_mark (`mark`);
ALTER TABLE `role`
  ADD UNIQUE INDEX index_unique_name (`name`);
...
ALTER TABLE `role_resource`
  ADD INDEX index_resource_id (`resource_id`);
